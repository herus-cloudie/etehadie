            ...
            // console.log(item.category)
              ...
              <Grid item xs={6} key={index}>
                <Card style={selectedJob ? {backgroundColor : '#cde8ffd9'} :  {backgroundColor : 'white'}} sx={{ padding: '0 20px 10px 20px', display: 'flex', gap: '10px', flexDirection: "column" }}>
                  ...
                  <div>شناسه اتحادیه : <span>{item.senf_code}</span></div>
                  <div>سمت جاری : <span>{item.position}</span></div>
                  <div>اتحادیه: <span>{item.workplace}</span></div>
                </Card>
              </Grid>
            )
          })}
        ...
        <DialogContent>
          <FormControl fullWidth>
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <TextField {...field} label="شناسه اتحادیه" value={newJob.senfCode} onChange={(e) => setNewJob({ ...newJob, senfCode: e.target.value })} />
              )}
            />
            <br />
            <Autocomplete
